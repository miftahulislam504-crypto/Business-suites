...
import { addSupplier, updateSupplier } from '@/lib/firestore'
...
export function SupplierForm({ supplier, onClose, onSaved }: Props) {
  ...
  async function handleSubmit() {
    if (!form.name.trim())  { toast.error('সাপ্লায়ারের নাম দাও'); return }
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-800 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-lg">{isEdit ? 'সাপ্লায়ার সম্পাদনা' : 'নতুন সাপ্লায়ার'}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div>
            ...
            <input value={form.name} onChange={e => set('name', e.target.value)}
              placeholder="সাপ্লায়ারের নাম" className="input-field" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">কোম্পানি</label>
            <input value={form.company} onChange={e => set('company', e.target.value)}
              placeholder="কোম্পানির নাম" className="input-field" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.address} onChange={e => set('address', e.target.value)}
              placeholder="সাপ্লায়ারের ঠিকানা" className="input-field" />
          </div>
          <div>
